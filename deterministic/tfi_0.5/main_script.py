...
h = .5  # strength of transverse field
...
def objective(weights):
    ...
    angles = features2[jnp.newaxis, :, :] \
        * jnp.conj(configs2)[:, jnp.newaxis, :]
...
if rgn:
    ...
    for iteration in range(iterations):
        ...
        if reset:
            print('Resetting')
            np.save('rgn_error_' + str(iteration) + '.npy', weights)
        ...
        print('Energy error: ', rgn_exact[iteration] / d - soln)
    ...
    np.save('rgn_hessian.npy', hessian_error)

# %%

# ============================
# Deterministic training -- LM
# ============================
...
if lm:
    ...
    for iteration in range(iterations):
        ...
        print('Energy error: ', lm_exact[iteration] / d - soln)
...
if gd:
    ...
    for iteration in range(iterations):
        ...
        if reset:
            print('Resetting')
            np.save('gd_error_' + str(iteration) + '.npy', weights)
